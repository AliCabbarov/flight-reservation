package ingress.common.service;
...
public final class JwtService {
    ...
    public Jws<Claims> parseToken(String token, HttpServletResponse response) {
        try {
            return Jwts.parser()
                    .verifyWith(key)
                    .build()
...
//            throw new RuntimeException("Invalid token");
    ...
    public String getUsername(String token, HttpServletResponse response) {
        return parseToken(token, response).getPayload().get("username", String.class);
    }
